...
![Markdown语法学习配图](http://p2p4htzmu.bkt.clouddn.com/peitu1.jpg)
...
* 在线编辑器 Dillinger
    ...
    ![Dillinger编辑器](http://p2p4htzmu.bkt.clouddn.com/peitu1-1.png "Dillinger编辑器")
...
* Windows 平台编辑器 MarkdownPad
    ...
    ![MarkdownPad](http://p2p4htzmu.bkt.clouddn.com/peitu1-2.png "MarkdownPad")

* OS X 平台编辑器 Mou

    [Mou](http://25.io/mou/ "Mou") 是 Mac 下杰出的 Markdown 编辑器，提供语法高亮、在线预览、同步滚动、全屏模式，支持自定保存、自动匹配，允许自定义主题，支持 CSS，HTML 和 PDF 导出等。

    ![Mou](http://p2p4htzmu.bkt.clouddn.com/peitu1-3.png "Mou")
...
* 多平台编辑器 vscode
    ...
    ![Markdown Preview Enhanced](http://p2p4htzmu.bkt.clouddn.com/peitu1-4.png "Markdown Preview Enhanced")
...
![段落](http://p2p4htzmu.bkt.clouddn.com/peitu1-5-1.png "段落")
...
![有缩进的段落](http://p2p4htzmu.bkt.clouddn.com/peitu1-6-1.png "有缩进的段落")
...
![空行展示](http://p2p4htzmu.bkt.clouddn.com/peitu1-7.png "空行展示")
...
* 类 Setext 形式是用底线的形式，利用 `=` （最高阶标题）和 `-` （第二阶标题），任何数量的 `=` 和 `-` 都可以有效果。如下图:
    ...
    ![类 Setext 形式](http://p2p4htzmu.bkt.clouddn.com/peitu1-8.png "类 Setext 形式")

* 类 Atx 形式则是在行首插入 1 到 6 个 `#` ，对应到标题 1 到 6 阶，这一种形式使用得相对多一些。这里需要注意一点就是在`#`和标题间最好隔一个空格，不然可能在有的编辑器下显示有问题，如下图:

    ![类 Atx 形式](http://p2p4htzmu.bkt.clouddn.com/peitu1-9.png "类 Atx 形式")

### 区块引用 Blockquotes

Markdown 标记区块引用是使用类似 email 中用 `>` 的引用方式。一共有两种方式，一种是在引用段落的每一行最前面加上`>`，还有一种方式是只在引用段落的第一行最前面加上`>`，这两种方式都能标记区块引用。

![区块引用](http://p2p4htzmu.bkt.clouddn.com/peitu1-10.png "区块引用")

* 区块引用里还可以可以嵌套（例如：引用内的引用），只要根据层次加上不同数量的 >
* 引用的区块内也可以使用其他的 Markdown 语法，包括标题、列表、代码区块等

![区块引用扩展](http://p2p4htzmu.bkt.clouddn.com/peitu1-11.png "区块引用扩展")
...
* 无序列表使用`*`、`+`或是`-`作为列表标记:
    ...
    ![无序列表](http://p2p4htzmu.bkt.clouddn.com/peitu1-12.png "无序列表")
...
* 有序列表则使用数字接着一个英文句点,这里需要注意的一点是你在列表标记上使用的数字并不会影响最后输出的 HTML 结果，就是说
    ...
    ![有序列表扩展](http://p2p4htzmu.bkt.clouddn.com/peitu1-13.png "有序列表扩展")

    这里还需注意的是两个独立的有序列表间，如果中间没有其他的标记元素作为分隔，那么这两个有序列表则被视为一个有序列表来进行标记的顺序递增。

    ![有序列表扩展](http://p2p4htzmu.bkt.clouddn.com/peitu1-14.png "有序列表扩展")
...
* 只需要标记一小段行内代码，这种情况需要使用反引号 ` 把需要标记的代码给包起来。注意，如果你要在代码区段内插入反引号，你可以用多个反引号来开启和结束代码区段。
    ...
    ![标记行内代码](http://p2p4htzmu.bkt.clouddn.com/peitu1-15.png "标记行内代码")
...
![代码块](http://p2p4htzmu.bkt.clouddn.com/peitu1-16-1.png "代码块")

### 分割线

华丽的分割线该如何来显示呢？我们只需在一行内输入三个以上的`*`或`-`或`_`来建立分割线，并且这一行中不能够包含其他东西。恩..你如果想要在符号间加几个空格也是可以的，这样也可以建立分割线。

![华丽的分割线](http://p2p4htzmu.bkt.clouddn.com/peitu1-17.png "华丽的分割线")
...
![链接](http://p2p4htzmu.bkt.clouddn.com/petu1-18.png "链接")
...
![强调](http://p2p4htzmu.bkt.clouddn.com/peitu1-19.png "强调")
...
总结了一下对 Markdown 的认识后，感觉自己对 Markdown 的理解又有了一些新的收获，注意到了一些以前忽视的东西。写一写总结感觉对自己帮助真的好大 :) ,希望我的总结也能够给你了解学习 Markdown 提供一点帮助，如果文章中出现错误或存在不严谨的地方，也请大家能够批评指出。
